 ...
 */
export const WEBEX_TEXT_LIMIT_BYTES = 7439;
export const WEBEX_SAFE_CHUNK_BYTES = 7200;
 ...
 */
export function mentionMarkdown(personId: string, displayName?: string): string {
 ...
 */
export function splitForWebex(
  text: string,
  limit: number = WEBEX_SAFE_CHUNK_BYTES
): string[] {
  if (!text) return [""];
  ...
  const appendToCurrent = (piece: string, sep: string) => {
  ...
  for (const paragraph of paragraphs) {
    ...
    for (const sentence of sentences) {
      if (appendToCurrent(sentence, " ")) continue;
      pushCurrent();
      ...
      for (const word of words) {
        if (appendToCurrent(word, " ")) continue;
        pushCurrent();
 ...
 */
export function transformMarkdownForWebex(md: string): string {
 ...
 */
export function trimToSafeMarkdownBoundary(s: string): string {
  if (!s) return s;
  let out = s;
 ...
 */
function splitElapsed(ms: number): { minutes: number; seconds: number } {
