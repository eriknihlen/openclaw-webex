 ...
 */
function withElapsedSuffix(line, startedAt) {
    const suffix = (0, formatters_1.formatElapsedShort)(Date.now() - startedAt);
    return {
...
function buildProgressReplyOptions(progress, verbosity, streamReasoning) {
    ...
    return {
        ...
        onItemEvent: (evt) => {
            ...
            const line = label && label !== "exec" && label !== "generic"
                ? { text: `${label}: ${clean}`, markdown: `**${(0, formatters_1.escapeMarkdown)(label)}** \`${q}\`` }
        ...
        onCommandOutput: (evt) => {
            ...
            if (command) {
                ...
                const line = withElapsedSuffix({ text: `Running: ${clean}`, markdown: `**Running** \`${(0, formatters_1.escapeMarkdown)(clean)}\`` }, progress.startedAt);
                progress.update(line.text, line.markdown);
            }
            else if (evt.name && evt.name !== "exec") {
                const line = withElapsedSuffix({ text: `Running ${evt.name}…`, markdown: `Running *${(0, formatters_1.escapeMarkdown)(evt.name)}*…` }, progress.startedAt);
                progress.update(line.text, line.markdown);
            }
        },
        onPlanUpdate: ({ title }) => {
 ...
 */
function resolveWorkspaceRootForMedia() {
...
async function deliverReplyMedia(opts) {
    ...
    const mediaItems = payload.mediaUrls && payload.mediaUrls.length > 0
        ? payload.mediaUrls
        : payload.mediaUrl
            ? [payload.mediaUrl]
            : [];
    ...
    console.log(`[webex:${accountId}] reply media: ${mediaItems.length} item(s), first=${firstBasename}`);
    ...
    for (const media of mediaItems) {
        ...
        try {
            ...
            const localPath = (0, outbound_file_guard_1.resolveAllowedOutboundFile)(localCandidate, config);
            await sender.sendLocalFile({
            ...
            console.warn(`[webex:${accountId}] media send failed for "${media}": ${err instanceof Error ? err.message : err}`);
        }
    }
}
/**
                    ...
                    if (payload.text) {
                        ...
                        if (isCommandTurn && commandName) {
                            ...
                            const elapsedLabel = showPlaceholder
                                ? (0, formatters_1.formatElapsed)(Date.now() - startedAt)
                                : undefined;
...
async function processAttachmentActionAsync(opts) {
    ...
    if (action.inputs.intent === "aiops-cve-evaluate") {
        ...
        if (!components || !currentValid || !productValid) {
            ...
            console.warn(`[webex:${account.accountId}] dropped aiops-cve-evaluate action ${action.id}: invalid components (${JSON.stringify(rawComponents)}), current (${JSON.stringify(rawCurrent)}), or product (${JSON.stringify(rawProduct)})`);
            ...
            try {
                await sender.send({
                    ...
                    content: {
                        text: `⚠️ ${dropText}`,
                        markdown: `⚠️ ${(0, formatters_1.escapeMarkdown)(dropText)}`,
                    },
                    parentId: action.messageId,
                });
            }
            catch (err) {
                console.warn(`[webex:${account.accountId}] aiops-cve-evaluate malformed-payload notice post failed: ${err instanceof Error ? err.message : err}`);
            }
            return;
        }
        ...
        try {
            ...
            networkError = true;
            console.warn(`[webex:${account.accountId}] aiops-cve-evaluate request errored for components=${components.join(",")} (actor=${actor}): ${err instanceof Error ? err.message : err}`);
        }
        const ts = formatUtcTimestamp(new Date());
        const safeActor = (0, formatters_1.escapeMarkdown)(truncate(actor, 80));
        if (ok) {
            ...
            const results = Array.isArray(responseFields?.results)
                ...
                : undefined;
            if (!results) {
                console.warn(`[webex:${account.accountId}] aiops-cve-evaluate got HTTP ${httpStatus} with an absent/unparseable response body for components=${components.join(",")} (actor=${actor})`);
                ...
                try {
                    ...
                    console.warn(`[webex:${account.accountId}] aiops-cve-evaluate failure notice post failed (actor=${actor}): ${err instanceof Error ? err.message : err}`);
                }
                return;
            }
            ...
            const bucketParts = [];
            if (noTarget > 0)
                bucketParts.push(`${noTarget} no upgrade target (${noTargetIds.join(", ")})`);
            if (notFound > 0)
                bucketParts.push(`${notFound} not in dashboard (${notFoundIds.join(", ")})`);
            ...
            if (queued === 0) {
                ...
                try {
                    await sender.send({
                        ...
                        content: {
                            text: `ℹ️ ${infoPlain}`,
                            markdown: `ℹ️ ${(0, formatters_1.escapeMarkdown)(infoPlain)}`,
                        },
                        parentId: action.messageId,
                    });
                }
                catch (err) {
                    console.warn(`[webex:${account.accountId}] aiops-cve-evaluate info notice post failed (actor=${actor}): ${err instanceof Error ? err.message : err}`);
                }
                return;
            }
            ...
            try {
                await sender.send({
                    ...
                    content: {
                        text: `⚙️ ${confirmPlain}`,
                        markdown: `⚙️ ${(0, formatters_1.escapeMarkdown)(confirmPlain)}`,
                    },
                    parentId: action.messageId,
                });
            }
            catch (err) {
                console.warn(`[webex:${account.accountId}] aiops-cve-evaluate confirmation post failed (actor=${actor}): ${err instanceof Error ? err.message : err}`);
            }
        }
        else if (httpStatus === 401 || httpStatus === 403) {
            ...
            console.warn(`[webex:${account.accountId}] aiops-cve-evaluate auth failure (HTTP ${httpStatus}) for components=${components.join(",")} (actor=${actor})`);
            ...
            try {
                ...
                console.warn(`[webex:${account.accountId}] aiops-cve-evaluate auth-failure notice post failed (actor=${actor}): ${err instanceof Error ? err.message : err}`);
            }
        }
        else {
            ...
            console.warn(`[webex:${account.accountId}] aiops-cve-evaluate NOT started for components=${components.join(",")} (actor=${actor}, httpStatus=${httpStatus ?? "n/a"}${networkError ? ", network error" : ""})`);
